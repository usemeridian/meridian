...
5. Run `meridian whoami --setup` so I can provide my Slack user ID (used for @mentions and DMs)
6. Ask me if I want to set up team context (/init-team) for shared journals,
   digests, and product state
7. Let me know about opt-in telemetry (set MERIDIAN_TELEMETRY=true to enable)
```
...
```
bash ~/.claude/meridian/backup/setup.sh <your-repo-url>
```
...
https://github.com/usemeridian/meridian

To install a specific version:
```
npm install -g meridian-dev@1.1.0
meridian init
```

Or via the shell installer with a pinned version:
```
MERIDIAN_VERSION=v1.1.0 bash <(curl -fsSL https://raw.githubusercontent.com/usemeridian/meridian/main/install.sh)
```
