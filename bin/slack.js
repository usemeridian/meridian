...
async function deliver(webhookUrl, digestContent, personaName, dateRange, options) {
  ...
  const label = personaName === 'unified' ? 'Meridian' : capitalize(personaName);
  ...
  if (process.env.MERIDIAN_SIMULATE === '1') {
    const digestsDir = path.join(HOME, '.claude', 'meridian', 'digests');
  ...
  for (let i = 0; i < personaIds.length; i++) {
    ...
    try {
      ...
      const digestFile = path.join(
        HOME, '.claude', 'meridian', 'digests', persona, `${toDate}.md`
      );
