...
function migrateFromJson(sqliteBackend, storePath) {
  ...
  if (totalEntries > 0) {
    console.error(`[meridian] Migrated ${totalEntries} entries from JSON to SQLite`);
  }
}
 ...
 * Honors MERIDIAN_STORAGE_BACKEND env var ('json' or 'sqlite').
...
function getBackend(storePath) {
  ...
  const forced = process.env.MERIDIAN_STORAGE_BACKEND;
