...
const EXCLUDE_REPOS_RAW = (process.env.MERIDIAN_EXCLUDE_REPOS || '')
...
function collectSignals(sinceDate, signalsDir) {
  signalsDir = signalsDir || path.join(HOME, '.claude', 'meridian', 'signals');
...
function loadPreviousDigest(personaId, currentDate) {
  const digestDir = path.join(HOME, '.claude', 'meridian', 'digests', personaId);
...
async function generateDigest(config, personaIds, sinceDate, onProgress) {
  ...
  // Filter out content mentioning excluded repos (MERIDIAN_EXCLUDE_REPOS)
  ...
  const digestDir = path.join(HOME, '.claude', 'meridian', 'digests');
  const files = [];
  ...
  for (let i = 0; i < personaIds.length; i++) {
    ...
    // Debug: dump prompt if MERIDIAN_DEBUG_PROMPT is set
    if (process.env.MERIDIAN_DEBUG_PROMPT) {
  ...
  const combinedContent = personaResults
    .map((p) => {
      const title = p.id === 'unified' ? 'Meridian Digest' : `${p.id.charAt(0).toUpperCase() + p.id.slice(1)} Digest`;
      return `# ${title}\n\n${p.content}`;
    })
    .join('\n\n---\n\n');
...
async function configure() {
  ...
  // Step 1b: API key — save to ~/.claude/meridian/.env
  if (llmConfig.api_key_env) {
    const existing = process.env[llmConfig.api_key_env];
    if (existing) {
      console.log(`\n${llmConfig.api_key_env} found in environment.`);
      const save = await ask('Save it to meridian config so it works from any terminal? (Y/n): ');
