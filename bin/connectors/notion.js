...
function isSimulation() {
  return process.env.MERIDIAN_SIMULATE === '1';
}

function getFixturesDir() {
  return process.env.MERIDIAN_SIM_NOTION_FIXTURES
    || process.env.MERIDIAN_SIM_FIXTURES
    || '';
}
...
async function pull(config, since) {
  ...
  if (!token && !isSimulation()) {
    throw new Error('Notion token is missing. Run "meridian pull notion --configure" to set it up.');
  }
