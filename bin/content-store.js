...
const DEFAULT_STORE_PATH = HOME ? path.join(HOME, '.claude', 'meridian', 'content-store') : null;
...
const EXCLUDE_REPOS = (process.env.MERIDIAN_EXCLUDE_REPOS || '')
 ...
 * @param {boolean} [options.embeddings] - Generate embeddings (default: true if OPENAI_API_KEY or MERIDIAN_SIMULATE)
...
async function indexConversations(options = {}) {
  ...
  const llmConfig = options.llmConfig || {
    provider: 'anthropic',
    model: process.env.MERIDIAN_EXTRACTION_MODEL || 'claude-haiku-4-5-20251001',
...
const ONBOARDING_PROMPT = `You are Meridian, generating an onboarding context pack for a new engineer joining a repo. Synthesize the provided decision trail entries into a structured onboarding document.
