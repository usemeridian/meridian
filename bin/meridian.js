...
const MERIDIAN_DIR = path.join(HOME, '.claude', 'meridian');

// Auto-load .env from meridian config dir BEFORE requiring modules
...
const CLI_USER = process.env.MERIDIAN_AUTHOR || 'system';
const TEAM_FILE = path.join(MERIDIAN_DIR, 'team.json');
const PROFILE_FILE = path.join(MERIDIAN_DIR, 'profile.json');
const CONNECTORS_FILE = path.join(MERIDIAN_DIR, 'connectors.json');

// ── Persona config resolution ────────────────────────────────────────────────
// User config lives at ~/.claude/meridian/personas.json (Claude Code) or
// ~/.ai-memory/meridian/personas.json (Cursor/generic). Falls back to the
// bundled default in templates/personas.json.

function getPersonasConfigPath() {
  const candidates = [
    path.join(HOME, '.claude', 'meridian', 'personas.json'),
    path.join(HOME, '.ai-memory', 'meridian', 'personas.json'),
  ];
...
function ensureUserConfig() {
  ...
  const dest = path.join(HOME, '.claude', 'meridian', 'personas.json');
...
function runPersonas(args) {
  ...
  if (addIdx !== -1) {
    ...
    if (!id || !name) {
      console.error('Usage: meridian personas --add <id> <name> [description]');
      process.exit(1);
    }
  ...
  if (removeIdx !== -1) {
    ...
    if (!id) {
      console.error('Usage: meridian personas --remove <id>');
      process.exit(1);
    }
    const configPath = ensureUserConfig();
    const data = readPersonas();
  ...
  console.log(`Config: ${configPath}`);
  if (isDefault) {
    console.log('Run "meridian personas --add" or "meridian personas --reset" to create a user config.');
  }
  console.log('');
}
...
function runAutopilot(args) {
  ...
  if (sub === 'status') {
    ...
    if (!args[1]) {
      console.error('Usage: meridian autopilot enable <persona-id>');
      process.exit(1);
    }
    autopilotEnable(args[1]);
  } else if (sub === 'disable') {
    if (!args[1]) {
      console.error('Usage: meridian autopilot disable <persona-id>');
      process.exit(1);
    }
    autopilotDisable(args[1]);
  } else {
    console.error(`Unknown autopilot subcommand: ${sub}`);
    console.error('Usage: meridian autopilot [status|enable|disable] [persona-id]');
    process.exit(1);
  }
}
...
async function teamCreate() {
  ...
  console.log(`  meridian team join ${id}`);
  console.log('');
}
...
async function teamJoin(args) {
  ...
  if (!teamId) {
    ...
    console.error('Usage: meridian team join <team-id>');
    process.exit(1);
  }
  ...
  if (profile) {
    ...
    console.log("  Run 'meridian whoami --setup' to register in the team directory.");
  }
  console.log('');
}
...
function teamStatus() {
  ...
  if (!team) {
    console.log('');
    console.log("No team configured. Run 'meridian team create' or 'meridian team join <id>'");
    console.log('');
    return;
  }
  ...
  if (!repoPath) {
    console.log('');
    console.log("(Run 'meridian context init' to see team members)");
  } else {
...
function whoamiShow() {
  ...
  if (!profile) {
    console.log('');
    console.log("No profile configured. Run 'meridian whoami --setup' to create one.");
    console.log('');
    return;
  }
...
async function runPull(args) {
  ...
  if (args.includes('--all')) {
    ...
    if (channels.length === 0) {
      console.log('No channels configured. Run "meridian pull <channel> --configure" first.');
      return;
    }
  ...
  if (!channel) {
    console.error('Usage: meridian pull <channel> [--since YYYY-MM-DD] [--configure]');
    console.error('       meridian pull --all');
  ...
  if (addIdx !== -1) {
    ...
    if (!repoArg || !/^[A-Za-z0-9._-]+\/[A-Za-z0-9._-]+$/.test(repoArg)) {
      console.error(`Usage: meridian pull ${channel} --add-repo owner/repo`);
      process.exit(1);
    }
    const [owner, repo] = repoArg.split('/');
    const config = readConnectorsConfig();
    if (!config[channel]) {
      console.error(`${channel} is not configured. Run "meridian pull ${channel} --configure" first.`);
      process.exit(1);
    }
  ...
  if (removeIdx !== -1) {
    ...
    if (!repoArg || !/^[A-Za-z0-9._-]+\/[A-Za-z0-9._-]+$/.test(repoArg)) {
      console.error(`Usage: meridian pull ${channel} --remove-repo owner/repo`);
      process.exit(1);
    }
    const [owner, repo] = repoArg.split('/');
    const config = readConnectorsConfig();
...
function runSignals() {
  ...
  if (channels.length === 0) {
    ...
    console.log('  meridian pull <channel> --configure');
    console.log('');
    return;
  }
...
async function runDigest(args) {
  ...
  if (apiKeyEnv && !process.env[apiKeyEnv] && process.env.MERIDIAN_SIMULATE !== '1') {
    console.error(`Error: ${apiKeyEnv} is not set.`);
    console.error('');
    console.error('Fix: run "meridian digest --configure" to save your API key,');
  ...
  if (deliver) {
    ...
    if (!webhookUrl) {
      ...
      console.error('Set MERIDIAN_SLACK_WEBHOOK env var or run "meridian digest --configure".');
      process.exit(1);
    }
...
function buildRepoToTeamResolver() {
  ...
  // Build a lookup: scan all known repo paths for .claude/meridian.json bindings
  const repoToTeamMap = {};
    ...
    try {
      ...
      for (const org of orgs) {
        ...
        for (const repo of repos) {
          const bindingFile = path.join(orgDir, repo, '.claude', 'meridian.json');
          try {
...
async function runIndexConversationsWithExport(args, detectShifts = false) {
  ...
  try {
    ...
    if (detectShifts && stats.pendingExports && stats.pendingExports.length > 0) {
      ...
      const llmConfig = {
        provider: 'anthropic',
        model: process.env.MERIDIAN_SHIFT_MODEL || 'claude-haiku-4-5-20251001',
...
async function runSearchJournals(args) {
  ...
  if (!query) {
    console.error('Usage: meridian search-journals <query> [--text] [--limit N] [--repo <name>] [--since YYYY-MM-DD] [--until YYYY-MM-DD] [--drifted]');
    process.exit(1);
  }
 ...
 * Usage: meridian journal migrate [--dir <path>] [--author <slug>] [--dry-run]
 */
function journalMigrate(args) {
  ...
  if (!author) {
    console.error('Could not determine author. Run "meridian whoami --setup" or pass --author <slug>.');
    process.exit(1);
  }
  ...
  if (!dryRun && count > 0) {
    console.log('Run "meridian reindex --journals-only" to update the content store.');
  }
}
 ...
 * Usage: meridian journal split [--dir <path>] [--dry-run]
 */
function journalSplitByTeam(args) {
  ...
  const config = readContextConfig();
  if (!config.teams) {
    console.error('No multi-team config found. Run "meridian context add" first.');
    process.exit(1);
  }
  ...
  if (!dryRun && splitCount > 0) {
    console.log('Run "meridian journal sync" to push split files to team repos.');
  }
}
 ...
 * Usage: meridian journal sync [--dir <path>] [--since YYYY-MM-DD]
 */
...
function runStatus(args) {
  ...
  if (doWrite) {
    const globalPath = process.env.MERIDIAN_GLOBAL_STATE || rebuildStatus.DEFAULT_GLOBAL_STATE;
    try {
...
async function runBot(args) {
  ...
  const config = readConnectorsConfig();
  if (!config.slack_bot) {
    console.error('Slack bot is not configured. Run "meridian bot --configure" first.');
    process.exit(1);
  }
  ...
  if (!process.env[botTokenEnv]) {
    ...
    console.error('Run "meridian bot --configure" to save your tokens.');
    process.exit(1);
  }
  ...
  if (!index || index.entryCount === 0) {
    console.log('Warning: Content store is empty. Run "meridian index-journals" first for best results.');
  ...
  if (llmConfig.api_key_env && !process.env[llmConfig.api_key_env]) {
    ...
    console.error('Run "meridian bot --configure" or set the key in your environment.');
    process.exit(1);
  }

  console.log('Starting Meridian Slack bot...');
...
const CONTEXT_CONFIG_FILE = path.join(MERIDIAN_DIR, 'context.json');
...
/**
 * Read repo-level team binding from .claude/meridian.json in cwd.
 * @returns {string|null} Team ID or null
 */
function readRepoTeamBinding() {
  const bindingFile = path.join(process.cwd(), '.claude', 'meridian.json');
  const binding = readJSONFile(bindingFile);
  return binding ? binding.team_id : null;
}

/**
 * Write repo-level team binding to .claude/meridian.json in cwd.
 * Also ensures .claude/meridian.json is gitignored.
 * @param {string} teamId
 */
function writeRepoTeamBinding(teamId) {
  const claudeDir = path.join(process.cwd(), '.claude');
  fs.mkdirSync(claudeDir, { recursive: true });
  const bindingFile = path.join(claudeDir, 'meridian.json');
  ...
  if (fs.existsSync(gitignorePath)) {
    ...
    if (!content.includes('.claude/meridian.json')) {
 ...
 * Priority: 1) repo-level .claude/meridian.json team binding
...
function stampMemberVersion(teamContextPath) {
  ...
  if (member.meridian_version === version && member.last_active && member.last_active.slice(0, 10) === now.slice(0, 10)) {
    return;
  }

  member.meridian_version = version;
  member.last_active = now;
  fs.writeFileSync(memberFile, JSON.stringify(member, null, 2) + '\n');
}

/**
 * Read min_version from the team-context repo's meridian.json.
...
function getTeamMinVersion(teamId) {
  ...
  const sharedConfig = readJSONFile(path.join(repoPath, 'meridian.json'));
...
function checkMinVersion(teamId) {
  ...
  const installed = telemetry.getMeridianVersion();
  return {
...
function syncMemberToRegistry(profile, teamId) {
  ...
  const memberData = {
    ...
    teamId,
    meridian_version: telemetry.getMeridianVersion(),
...
function syncWebhookToTeamContext(digestConfig) {
  ...
  const sharedConfigPath = path.join(repoPath, 'meridian.json');
  ...
  try {
    const { execSync } = require('child_process');
    execSync(
      `git -C "${repoPath}" add meridian.json && git -C "${repoPath}" commit -m "Update shared Slack webhook for team announcements" && git -C "${repoPath}" push`,
      { stdio: 'pipe' }
    );
  } catch {
...
async function contextInit(args) {
  ...
  console.log('');
  console.log('Run "meridian context sync" in any repo to pull context files.');
}

function contextSync() {
  const repoPath = getTeamContextPath();
  if (!repoPath) {
    console.error('No team context repo configured. Run "meridian context init <path>" first.');
    process.exit(1);
  }
  ...
  if (fs.existsSync(gitignorePath)) {
    ...
    if (!content.split('\n').some(line => line.trim() === entry)) {
      fs.appendFileSync(gitignorePath, `\n# Shared team context (synced by meridian)\n${entry}\n`);
  ...
  if (fs.existsSync(claudeMdPath)) {
    ...
    if (!claudeMd.includes('.claude/context/')) {
      const contextBlock = '\n\n## Shared Team Context\n\n' +
        'Context files synced from the team context repo (run `meridian context sync` to update):\n' +
...
function contextShow() {
  ...
  if (config.teams && Object.keys(config.teams).length > 0) {
    ...
    console.log('  Not configured. Run "meridian context init <path>" to set up.');
  }
  ...
  if (fs.existsSync(localDir)) {
    ...
    console.log('\nNo local context in this repo. Run "meridian context sync" to pull.');
  }
}
...
function contextAdd(args) {
  ...
  if (!teamId || !repoPath) {
    console.error('Usage: meridian context add <team-id> <path>');
    console.error('Example: meridian context add a1b2c3d4 ~/repos/Acme/team-context');
    process.exit(1);
  }
  ...
  // Try to read team name from the repo's meridian.json
  const sharedConfig = readJSONFile(path.join(resolved, 'meridian.json')) || {};
...
function contextBind(args) {
  ...
  if (!teamId) {
    ...
    if (binding && config.teams && config.teams[binding]) {
      ...
      console.log('Usage: meridian context bind <team-id>');
    }
    return;
  }
...
function contextList() {
  ...
  if (!config.teams || Object.keys(config.teams).length === 0) {
    console.log('No teams configured. Run "meridian context init <path>" to set up.');
    return;
  }
  ...
  console.log('  meridian context add <team-id> <path>   Register a new team');
  console.log('  meridian context bind <team-id>         Bind this repo to a team');
  console.log('  meridian context default <team-id>      Change default team');
}

function contextSetDefault(args) {
  const teamId = args[0];
  if (!teamId) {
    console.error('Usage: meridian context default <team-id>');
    process.exit(1);
  }
...
function runPrompts(args) {
  ...
  const teamDir = process.env.MERIDIAN_TEAM_CONTEXT_DIR || '';
  ...
  console.log(`\nRun "meridian prompts <name>" to view a specific prompt.`);
}
...
function deployStatus() {
  ...
  if (!fs.existsSync(deployDir)) {
    console.log('No deploy/ directory found. Run "meridian deploy init" first.');
    process.exit(1);
  }
  ...
  const optional = ['MERIDIAN_TENANT_ID', 'SLACK_DIGEST_CHANNEL', 'MERIDIAN_ENCRYPTION_KEY'];
...
function startHealthServer() {
  const port = parseInt(process.env.MERIDIAN_HEALTH_PORT || '3141', 10);
  const server = http.createServer((req, res) => {
    if (req.url === '/healthz' && req.method === 'GET') {
      // Enrich with index freshness
      const storePath = process.env.MERIDIAN_STORE_PATH || contentStore.DEFAULT_STORE_PATH;
...
async function runStart() {
  const mode = process.env.MERIDIAN_MODE || 'all-in-one';
  console.log(`Meridian starting in ${mode} mode`);
  ...
  switch (mode) {
    ...
    default:
      console.error(`Unknown MERIDIAN_MODE: ${mode}`);
...
async function runStartWorker() {
  const job = process.env.MERIDIAN_JOB;
  if (!job) {
    console.error('MERIDIAN_JOB is required in worker mode.');
...
function runStartScheduler() {
  ...
  const digestSchedule = process.env.MERIDIAN_DIGEST_SCHEDULE || '0 12 * * *';
  const signalSchedule = process.env.MERIDIAN_SIGNAL_SCHEDULE || '0 6 * * *';
  ...
  const reindexSchedule = process.env.MERIDIAN_REINDEX_SCHEDULE || '0 * * * *';
 ...
 * Skipped silently if MERIDIAN_TEAM_CONTEXT_DIR is not set or not a git repo.
 */
...
async function indexJournalsIfAvailable() {
  const journalDir = process.env.MERIDIAN_JOURNALS_DIR || '/data/journals';
  if (!fs.existsSync(journalDir)) {
...
async function indexConversationsIfAvailable() {
  const projectsDir = process.env.MERIDIAN_CONVERSATIONS_DIR || contentStore.DEFAULT_PROJECTS_DIR;
...
async function indexSignalsIfAvailable() {
  const signalsDir = process.env.MERIDIAN_SIGNALS_DIR || contentStore.DEFAULT_SIGNALS_DIR;
...
function ensureContainerConfig() {
  ...
  if (!config.digest && process.env.ANTHROPIC_API_KEY) {
    config.digest = {
      llm: {
        provider: 'anthropic',
        model: process.env.MERIDIAN_LLM_MODEL || 'claude-sonnet-4-5-20250929',
        api_key_env: 'ANTHROPIC_API_KEY',
      },
      lookback_days: 7,
      store_path: process.env.MERIDIAN_STORE_PATH || contentStore.DEFAULT_STORE_PATH,
      journal_dir: process.env.MERIDIAN_JOURNALS_DIR || '/data/journals',
      signals_dir: process.env.MERIDIAN_SIGNALS_DIR || contentStore.DEFAULT_SIGNALS_DIR,
      team_context_dir: process.env.MERIDIAN_TEAM_CONTEXT_DIR || '',
      slack: {
        webhook_url: process.env.MERIDIAN_SLACK_WEBHOOK || '',
  ...
  if (!config.slack_bot && process.env.SLACK_BOT_TOKEN) {
    config.slack_bot = {
      ...
      mode: process.env.MERIDIAN_BOT_MODE || 'local',
      store_path: process.env.MERIDIAN_STORE_PATH || contentStore.DEFAULT_STORE_PATH,
      journal_dir: process.env.MERIDIAN_JOURNALS_DIR || '/data/journals',
      llm: {
        provider: 'anthropic',
  ...
  if (!config.github && process.env.GITHUB_TOKEN) {
    const repos = process.env.MERIDIAN_GITHUB_REPOS;
    if (repos) {
      config.github = {
        transport: 'https',
  ...
  if (!config.intercom && process.env.INTERCOM_TOKEN) {
    const tagFilter = process.env.MERIDIAN_INTERCOM_TAGS;
    config.intercom = {
      transport: 'https',
  ...
  if (!config.notion && process.env.NOTION_TOKEN) {
    const databases = process.env.MERIDIAN_NOTION_DATABASES;
    config.notion = {
      transport: 'https',
...
function runMembers(args) {
  const config = readContextConfig();
  const currentVersion = telemetry.getMeridianVersion();
  const doJson = args.includes('--json');
  const doSetMinVersion = args.includes('--set-min-version');

  // meridian members --set-min-version 1.8.28
  if (doSetMinVersion) {
    const vIdx = args.indexOf('--set-min-version');
    const version = args[vIdx + 1];
    if (!version || !/^\d+\.\d+\.\d+$/.test(version)) {
      console.error('Usage: meridian members --set-min-version <X.Y.Z>');
      process.exit(1);
    }
    const repoPath = getTeamContextPath();
    if (!repoPath) {
    ...
    try {
      spawnSync('git', ['add', 'meridian.json'], { cwd: repoPath, stdio: 'pipe' });
      spawnSync('git', ['commit', '-m', `Set minimum Meridian version to v${version}`], { cwd: repoPath, stdio: 'pipe' });
  ...
  for (const teamId of teamIds) {
    ...
    for (const file of fs.readdirSync(membersDir).filter(f => f.endsWith('.json'))) {
      ...
      const version = member.meridian_version || 'unknown';
  ...
  if (allMembers.length === 0) {
    ...
    console.log('Run "meridian whoami --setup" to create your profile.');
    return;
  }
  ...
  if (outdatedCount > 0) {
    ...
    console.log(`  ${outdatedCount} member(s) need to run: npm update -g meridian-dev`);
  }
}

/**
 * Check min_version at session start. Called from the session-start hook
 * via `meridian check-version`. Prints a warning and fires telemetry
...
function runCheckVersion() {
  ...
  console.error(`\x1b[33m⚠ Meridian v${result.installed} is below team minimum v${result.required}\x1b[0m`);
  console.error('  Run: npm update -g meridian-dev');
...
const COMMANDS = {
  start: {
    desc: 'Start Meridian in container mode (reads MERIDIAN_MODE env var)',
    run: () => runStart(),
  },
  init: {
  ...
  update: {
    desc: 'Update Meridian to latest version',
    run: (args) => {
      // Step 1: Pull latest from npm
      const skipNpm = args.includes('--skip-npm');
      if (!skipNpm) {
        console.log('Updating meridian-dev from npm...');
        const npmResult = spawnSync('npm', ['update', '-g', 'meridian-dev'], {
          stdio: 'inherit',
        });
        if (npmResult.error || (npmResult.status && npmResult.status !== 0)) {
          console.error('npm update failed. Try running: npm update -g meridian-dev');
          console.error('Then re-run: meridian update --skip-npm');
          process.exit(1);
        }
      }

      // Step 2: Re-run setup in update mode with the (now current) version
      const versionFile = path.join(HOME, '.claude', 'meridian', '.meridian-version');
      let oldVersion = '';
      try {
      ...
      const env = { ...process.env };
      if (oldVersion) env.MERIDIAN_OLD_VERSION = oldVersion;
      if (newVersion) env.MERIDIAN_NEW_VERSION = newVersion;
  ...
  whoami: {
    desc: 'Show or set up your Meridian profile and personas',
  ...
  version: {
    desc: 'Print installed Meridian version',
    run: () => {
      const versionFile = path.join(HOME, '.claude', 'meridian', '.meridian-version');
      try {
        ...
        try {
          const pkg = require(path.join(ROOT, 'package.json'));
          console.log(`Meridian v${pkg.version} (from package.json)`);
        } catch (e) {
          console.error('Version unknown (no .meridian-version file found)');
          process.exit(1);
        }
      }
    },
  ...
  pull: {
    desc: 'Pull signals from a channel (see "meridian signals" for available)',
  ...
  bot: {
    desc: 'Start the Meridian Slack bot for decision trail queries',
...
function showHelp() {
  ...
  console.log('  meridian team create           Create a new team');
  console.log('  meridian team join <id>        Join an existing team');
  console.log('  meridian team status           Show current team info');
  console.log('  meridian whoami                Show your profile');
  console.log('  meridian whoami --setup        Set up your profile and personas');
  console.log('');
  console.log('Personas:');
  console.log('  meridian personas              List configured personas');
  console.log('  meridian personas --add <id> <name> [description]');
  console.log('  meridian personas --remove <id>');
  console.log('  meridian personas --reset      Restore default personas');
  console.log('');
  console.log('Autopilot:');
  console.log('  meridian autopilot status              Which personas are human vs. autopilot');
  console.log('  meridian autopilot enable <persona>    Enable autopilot for a persona');
  console.log('  meridian autopilot disable <persona>   Disable autopilot for a persona');
  console.log('');
  console.log('Digests:');
  console.log('  meridian digest                           Generate all persona digests');
  console.log('  meridian digest --persona engineering     Generate one persona only');
  console.log('  meridian digest --deliver                 Generate + post to Slack');
  console.log('  meridian digest --since 2026-02-24        Override lookback period');
  console.log('  meridian digest --configure               Set up LLM + Slack config');
  console.log('  meridian journal [--last-week]            Plain-text journal summary');
  console.log('  meridian journal migrate [--dry-run]      Rename journals to YYYY-MM-DD-{author}.md');
  console.log('  meridian journal sync [--since DATE]      Copy authored journals to team-context repo');
  console.log('');
  console.log('Signal channels:');
  console.log('  meridian pull github             Pull GitHub signals');
  console.log('  meridian pull github --configure  Configure GitHub connector');
  console.log('  meridian pull github --since YYYY-MM-DD');
  console.log('  meridian pull github --add-repo owner/repo');
  console.log('  meridian pull github --remove-repo owner/repo');
  console.log('  meridian pull intercom           Pull Intercom signals');
  console.log('  meridian pull intercom --configure  Configure Intercom connector');
  console.log('  meridian pull --all              Pull all configured channels');
  console.log('  meridian signals                 Show configured channels');
  console.log('');
  console.log('Prompts:');
  console.log('  meridian prompts               List shared team prompts');
  console.log('  meridian prompts <name>        Show a specific prompt');
  console.log('');
  console.log('Bot:');
  console.log('  meridian bot                   Start the Slack bot (Socket Mode)');
  console.log('  meridian bot --configure       Set up Slack app tokens + LLM config');
  console.log('');
  console.log('Deployment:');
  console.log('  meridian deploy init           Scaffold Docker deployment files');
  console.log('  meridian deploy status         Check deployment configuration');
  console.log('');
  console.log('Members:');
  console.log('  meridian members                          Show team members with versions');
  console.log('  meridian members --json                   Machine-readable output');
  console.log('  meridian members --set-min-version X.Y.Z  Set minimum required version');
  console.log('');
  console.log('Status:');
  console.log('  meridian status                Print cross-project status table');
  ...
  console.log('  meridian index-journals --dir <path>           Custom journal directory');
  console.log('  meridian index-journals --no-embeddings        Skip embedding generation');
  console.log('  meridian search-journals <query>               Semantic search (needs OPENAI_API_KEY)');
  console.log('  meridian search-journals <query> --text        Full-text search (no API key)');
  console.log('  meridian search-journals <query> --repo meridian --since 2026-02-01');
  console.log('  meridian insights                              Show journal insights');
  console.log('  meridian insights --json                       JSON output');
  console.log('');
}
