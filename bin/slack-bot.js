...
const SIGNALS_DIR = path.join(MERIDIAN_DIR, 'signals');
...
function buildSynthesisPrompt() {
  ...
  return `You are Meridian, a team decision trail assistant. You answer questions about the team's engineering decisions, work sessions, and project history using the provided decision trail entries.
...
/**
 * Save a key=value pair to ~/.claude/meridian/.env.
 ...
 * Strips bot mention patterns like <@U12345> and @meridian prefix.
...
function extractQuery(text, botUserId) {
  ...
  // Trim before checking @meridian prefix so leading spaces don't prevent match
  query = query.trim();

  // Strip @meridian prefix (case-insensitive)
  query = query.replace(/^@meridian\s*/i, '');
...
function buildSignalSynthesisPrompt(channel) {
  const today = new Date().toISOString().slice(0, 10);
  return `You are Meridian, a team context assistant. You answer questions about business signals and trends using data pulled from ${channel}.
...
async function synthesizeAnswer(query, results, llmConfig, contentOpts, threadHistory) {
  if (results.length === 0) {
    return 'No matching entries found in the decision trail. Try rephrasing your question or indexing more journals with `meridian index-journals`.';
  }
  ...
  if (intent.type === 'signals' || intent.type === 'mixed') {
    ...
    if (intent.type === 'signals') {
      ...
      const text = `I don't have ${intent.channel} signal data available. ` +
        ...
        `To set it up: \`meridian pull ${intent.channel} --configure\`\n` +
        `To pull now: \`meridian pull ${intent.channel}\``;
      return { text, results: [] };
    }
...
async function start(config) {
  ...
  try {
    ...
    throw new Error(
      '@slack/bolt is not installed. Run "npm install @slack/bolt" in the Meridian directory.'
    );
  }

  // Ensure journal_dir has a default for configs saved before this field existed
  if (!config.journal_dir) {
    config.journal_dir = process.env.MERIDIAN_JOURNALS_DIR || '/data/journals';
  }
  ...
  if (!botToken) {
    throw new Error(
      `Missing ${botTokenEnv}. Run "meridian bot --configure" or set it in your environment.`
    );
  }
  if (!appToken) {
    throw new Error(
      `Missing ${appTokenEnv}. Run "meridian bot --configure" or set it in your environment.`
    );
  }
  ...
  const teamContextDir = config.team_context_dir || process.env.MERIDIAN_TEAM_CONTEXT_DIR || null;
  ...
  // This lets users ask follow-up questions without re-mentioning @Meridian.
  ...
  console.log('Meridian bot connected to Slack (Socket Mode)');
  if (botUserId) {
...
async function configure() {
  console.log('');
  console.log('Meridian Slack Bot Configuration');
  console.log('================================');
  console.log('');
  ...
  let llmConfig = {};
  const connectorsFile = path.join(MERIDIAN_DIR, 'connectors.json');
  try {
