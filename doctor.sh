...
PYEOF
        ...
        if [[ "$STRUCT_RESULT" == "OK" ]]; then
            ...
            info "Fix: run 'meridian update' to normalize hook structure"
            ISSUES=$((ISSUES + 1))
...
check_team_versions() {
    ...
    # Get installed version from .meridian-version file (written by setup.sh)
    local VERSION_FILE="$HOME/.claude/meridian/.meridian-version"
    ...
    if [ "$INSTALLED" = "unknown" ]; then
        info "Version not recorded yet (run meridian update to set)"
        return
    fi
    ...
    local CHECK_OUTPUT=""
    if [ -f "$HOME/repos/greg/meridian/bin/meridian.js" ]; then
        CHECK_OUTPUT=$(node "$HOME/repos/greg/meridian/bin/meridian.js" check-version 2>&1 || true)
    elif command -v meridian >/dev/null 2>&1; then
        CHECK_OUTPUT=$(meridian check-version 2>&1 || true)
    fi

    if echo "$CHECK_OUTPUT" | grep -q "below team minimum"; then
        local MIN_VER
        MIN_VER=$(echo "$CHECK_OUTPUT" | grep -o 'minimum v[0-9.]*' | head -1 | sed 's/minimum v//')
        warn "Below team minimum v$MIN_VER — run: npm update -g meridian-dev"
        ISSUES=$((ISSUES + 1))
    else
