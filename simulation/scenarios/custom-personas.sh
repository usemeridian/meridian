...
assert_file_exists "$MOCK_HOME/.claude/meridian/personas.json" "personas.json installed to user config"

# Verify installed personas.json has the default personas
assert_json_valid "$MOCK_HOME/.claude/meridian/personas.json" "installed personas.json is valid JSON"
assert_json_field "$MOCK_HOME/.claude/meridian/personas.json" ".personas[0].id" "product" "first default persona is product"
...
node "$KIT_DIR/bin/meridian.js" personas --add devops DevOps "CI/CD and infrastructure" >/dev/null 2>&1
assert_json_field "$MOCK_HOME/.claude/meridian/personas.json" ".personas[4].id" "devops" "CLI added devops persona"
assert_json_field "$MOCK_HOME/.claude/meridian/personas.json" ".personas[4].name" "DevOps" "CLI added devops with correct name"

# Test CLI personas --add rejects invalid ID (spaces, uppercase)
INVALID_EXIT=0
node "$KIT_DIR/bin/meridian.js" personas --add "Bad Name" "Bad" >/dev/null 2>&1 || INVALID_EXIT=$?
...
node "$KIT_DIR/bin/meridian.js" personas --remove devops >/dev/null 2>&1
PERSONAS_COUNT=$(python3 -c "import json, sys; print(len(json.load(open(sys.argv[1]))['personas']))" "$MOCK_HOME/.claude/meridian/personas.json" 2>/dev/null)
...
node "$KIT_DIR/bin/meridian.js" personas --add qa QA >/dev/null 2>&1
node "$KIT_DIR/bin/meridian.js" personas --reset >/dev/null 2>&1
RESET_COUNT=$(python3 -c "import json, sys; print(len(json.load(open(sys.argv[1]))['personas']))" "$MOCK_HOME/.claude/meridian/personas.json" 2>/dev/null)
