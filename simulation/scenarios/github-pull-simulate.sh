...
cat > "$TEST_HOME/.claude/meridian/connectors.json" << 'EOF'
{
  "github": {
    "transport": "simulate",
    "token": null,
    "repos": [
      { "owner": "acme-corp", "repo": "web-api" },
...
export MERIDIAN_SIM_FIXTURES="$SIM_DIR/fixtures/signals/github"

OUTPUT=$(node "$REPO_ROOT/bin/meridian.js" pull github --since 2026-02-20 2>&1) || true
...
SIGNALS_DIR="$TEST_HOME/.claude/meridian/signals/github"
TODAY_DATE=$(date +%Y-%m-%d)

# Check per-repo files exist
assert_file_exists "$SIGNALS_DIR/acme-corp/web-api/${TODAY_DATE}.md" "web-api signal file exists"
...
if [ -f "$SIGNALS_DIR/acme-corp/web-api/${TODAY_DATE}.md" ]; then
    SIGNAL_FILE="$SIGNALS_DIR/acme-corp/web-api/${TODAY_DATE}.md"
...
LAST_PULL=$(python3 -c "
import json
with open('$TEST_HOME/.claude/meridian/connectors.json') as f:
    data = json.load(f)
...
# Test 'meridian signals' command
echo ""
echo "  Signals command:"
SIGNALS_OUTPUT=$(node "$REPO_ROOT/bin/meridian.js" signals 2>&1) || true
