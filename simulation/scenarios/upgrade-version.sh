...
# Simulate an older version by writing a .meridian-version file
mkdir -p "$MOCK_HOME/.claude/meridian"
echo "0.9.0" > "$MOCK_HOME/.claude/meridian/.meridian-version"
...
MERIDIAN_OLD_VERSION="0.9.0" MERIDIAN_NEW_VERSION="$CURRENT_VERSION" \
    bash "$KIT_DIR/setup.sh" --tool claude-code --update >/dev/null 2>&1

# Verify the version file was updated
assert_file_exists "$MOCK_HOME/.claude/meridian/.meridian-version" "version file exists after upgrade"

VERSION_CONTENT=$(cat "$MOCK_HOME/.claude/meridian/.meridian-version" 2>/dev/null || echo "")
