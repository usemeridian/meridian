...
# Verify meridian CLI exists and shows help
CLI_OUTPUT=""
CLI_EXIT=0
CLI_OUTPUT=$(node "$KIT_DIR/bin/meridian.js" help 2>&1) || CLI_EXIT=$?
if [ "$CLI_EXIT" -eq 0 ]; then
    _pass "meridian CLI help exits 0"
else
    _fail "meridian CLI help exits 0" "exited $CLI_EXIT"
fi

if echo "$CLI_OUTPUT" | grep -qF "meridian"; then
    _pass "CLI help output mentions meridian"
else
    _fail "CLI help output mentions meridian" "output: $CLI_OUTPUT"
fi
