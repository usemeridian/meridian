...
#   1. npm install -g meridian-dev  (simulated — we use the local bin directly)
#   2. meridian init
#   3. meridian doctor
#   4. meridian version
...
# Unset all Meridian env vars to simulate truly clean machine
unset MERIDIAN_TELEMETRY 2>/dev/null || true
unset MERIDIAN_SIMULATE 2>/dev/null || true
unset MERIDIAN_TENANT_ID 2>/dev/null || true
unset MERIDIAN_EXCLUDE_REPOS 2>/dev/null || true
unset MERIDIAN_LLM_MODEL 2>/dev/null || true
unset MERIDIAN_AUTHOR 2>/dev/null || true
...
CLI="node $KIT_DIR/bin/meridian.js"

# ── Step 1: meridian help (first thing a curious user tries) ────────────────

echo ""
echo "Step 1: meridian help"
echo "---------------------"
...
if [ "$HELP_EXIT" -eq 0 ]; then
    _pass "meridian help exits 0"
else
    _fail "meridian help exits 0" "exited $HELP_EXIT"
fi
...
# ── Step 2: meridian version ────────────────────────────────────────────────

echo ""
echo "Step 2: meridian version"
echo "------------------------"
...
if [ "$VERSION_EXIT" -eq 0 ]; then
    _pass "meridian version exits 0"
else
    _fail "meridian version exits 0" "exited $VERSION_EXIT"
fi

# ── Step 3: meridian init (the main onboarding command) ─────────────────────

echo ""
echo "Step 3: meridian init"
echo "---------------------"
...
if [ "$INIT_EXIT" -eq 0 ]; then
    _pass "meridian init exits 0"
else
    _fail "meridian init exits 0" "exited $INIT_EXIT"
fi
...
# ── Step 4: meridian doctor (validate the install) ──────────────────────────

echo ""
echo "Step 4: meridian doctor"
echo "-----------------------"
...
if [ "$DOCTOR_EXIT" -eq 0 ]; then
    _pass "meridian doctor exits 0"
else
    _fail "meridian doctor exits 0" "exited $DOCTOR_EXIT — output: $DOCTOR_OUTPUT"
fi
...
# ── Step 6: meridian status ─────────────────────────────────────────────────

echo ""
echo "Step 6: meridian status"
echo "-----------------------"
...
if [ "$STATUS_EXIT" -le 1 ]; then
    _pass "meridian status doesn't crash"
else
    _fail "meridian status doesn't crash" "exited $STATUS_EXIT"
fi

# ── Step 7: meridian signals (should work with no config) ──────────────────

echo ""
echo "Step 7: meridian signals (no config)"
echo "-------------------------------------"
...
if [ "$SIGNALS_EXIT" -le 1 ]; then
    _pass "meridian signals handles no-config gracefully"
else
    _fail "meridian signals handles no-config gracefully" "exited $SIGNALS_EXIT"
fi

# ── Step 8: meridian search-journals (empty state) ─────────────────────────

echo ""
echo "Step 8: meridian search-journals (empty)"
echo "-----------------------------------------"
