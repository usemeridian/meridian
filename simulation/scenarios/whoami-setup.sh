...
|---------|------|--------|------|
| Meridian | ~/repos/meridian | Building v1.1 | Ship simulation harness |

## Memory Files (load on demand)

| File | When to load | Summary |
|------|-------------|---------|
| `meridian-arch.md` | meridian, architecture | Architecture decisions for Meridian |
...
| `~/repos/meridian/.claude/state.md` | Meridian dev state |
EOF
...
assert_file_contains "$MOCK_HOME/.claude/global-state.md" "Meridian" "active projects filled in"
