...
cat > "$TEST_HOME/.claude/meridian/connectors.json" << 'EOF'
{
  "github": {
    "transport": "simulate",
    "repos": [{ "owner": "acme-corp", "repo": "web-api" }],
    "last_pull": null
  },
  "digest": {
    "llm": {
...
cp "$SIM_DIR/fixtures/signals/github/"*.json "$TEST_HOME/.claude/meridian/signals/github/" 2>/dev/null || true

# Create a summary signal file
cat > "$TEST_HOME/.claude/meridian/signals/github/2026-02-28-summary.md" << 'SIGNAL_EOF'
...
## Meridian — GitHub Signal Connector
...
DIGESTS_DIR="$TEST_HOME/.claude/meridian/digests"
