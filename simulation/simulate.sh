#!/usr/bin/env bash
# Meridian — End-to-end simulation harness
...
if [ "$ACTION" = "run-one" ]; then
    echo ""
    echo -e "${BOLD}Meridian Simulation — Single Scenario${RESET}"
    echo "======================================"
    run_scenario "$TARGET"
    exit $?
fi
...
echo ""
echo -e "${BOLD}Meridian Simulation — Full Suite${RESET}"
echo "================================="
