...
## Technical Debt
- web-api: 3 hardcoded config values bypassing config-as-code pattern (PR #2465). Quick fix shipped but underlying config model needs cleanup.
...
- report-service nuget PAT cleanup (PRs #2474-#2486) touching build pipeline across 12 repos.

## CI/CD
- 2 workflow failures on main this period (analytics-service). Both related to SQL MI connection timeout.
- web-api: 14 runs, 0 failures. Clean.

## Code Quality
- PR #2471 merged without tests (data-pipeline batch endpoint). Coverage dropped 3%.
