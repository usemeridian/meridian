...
1. Install Meridian in the target repo.
2. Run `/init-memory` to set up state files and the session protocol.
...
1. Run `/init-team` to set up shared team state and journal aggregation.
...
Once the digest is posting, the next high-value step is the Slack bot. It lets anyone on the team ask `@meridian <question>` and get answers grounded in the team's journal entries and decision trail.

1. Create a Slack app with Socket Mode enabled (no public URL required — runs wherever you run it).
2. Run `meridian bot --configure` to save your Slack app token (`xapp-`), bot token (`xoxb-`), and LLM settings.
3. Index your journal entries: `meridian index-journals`.
4. Start the bot: `meridian bot`.
5. In Slack, mention the bot: `@meridian what did we decide about the retry logic?`
