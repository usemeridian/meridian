...
**Default: local polling via `meridian pull <channel>`.**
...
```
meridian pull github                    # Poll GitHub API, write signal files
meridian pull github --configure        # Interactive setup (transport, repos)
meridian pull github --add-repo o/r     # Add a repo to config
meridian pull github --remove-repo o/r  # Remove a repo from config
meridian pull notion                    # Poll Notion API
meridian pull linear                    # Poll Linear API (planned, not yet available)
meridian pull intercom                  # Poll Intercom API
meridian pull intercom --configure      # Interactive Intercom setup
meridian pull --all                     # Poll all configured channels
meridian signals                        # Show configured channels and last pull time
```

Output is per-repo markdown files at `~/.claude/meridian/signals/<channel>/<owner>/<repo>/<date>.md` plus a cross-repo rollup at `~/.claude/meridian/signals/<channel>/<date>-summary.md`. The digest generator reads these alongside journals when building persona views.
...
**Provider flexibility:** The LLM and embedding providers are currently hardcoded to Anthropic (Claude) and OpenAI respectively. Planned work will make these configurable, supporting alternative providers and local models.
