...
export MERIDIAN_SIM_NOTION_FIXTURES="$SCRIPT_DIR/simulation/fixtures/signals/notion"

PULL_OUTPUT=$(node "$SCRIPT_DIR/bin/meridian.js" pull notion --since 2026-02-20 2>&1) || true
assert_contains "pull output shows Pages" "Pages:" "$PULL_OUTPUT"

# Check signal file was created
TODAY_DATE=$(date +%Y-%m-%d)
SIGNAL_FILE="$TEST_HOME/.claude/meridian/signals/notion/${TODAY_DATE}.md"
...
PULL_ALL=$(node "$SCRIPT_DIR/bin/meridian.js" pull --all --since 2026-02-20 2>&1) || true
