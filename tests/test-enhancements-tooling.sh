#!/usr/bin/env bash
# Meridian — Tooling Enhancements Test Suite
...
echo ""
echo "Meridian — Tooling Enhancements Test Suite"
echo "================================================"
echo ""
...
assert_file_contains "$INSTALLER" "MERIDIAN_VERSION" \
    "install.sh supports MERIDIAN_VERSION env var"

assert_file_contains "$INSTALLER" 'VERSION="${MERIDIAN_VERSION:-main}"' \
...
assert_file_contains "$ROOT_DIR/VERSIONS.md" "MERIDIAN_VERSION" \
    "VERSIONS.md documents MERIDIAN_VERSION usage"

assert_file_contains "$ROOT_DIR/VERSIONS.md" "git tag -a" \
    "VERSIONS.md documents tagging process"

assert_file_contains "$ROOT_DIR/BOOTSTRAP_PROMPT.md" "MERIDIAN_VERSION" \
...
# install.sh detects existing installation via .meridian-version
assert_file_contains "$INSTALLER" ".meridian-version" \
    "install.sh checks .meridian-version file"
...
# setup.sh writes .meridian-version after successful install
assert_file_contains "$ROOT_DIR/setup.sh" ".meridian-version" \
    "setup.sh writes .meridian-version file"

# setup.sh shows upgrade messaging
assert_file_contains "$ROOT_DIR/setup.sh" "MERIDIAN_OLD_VERSION" \
    "setup.sh reads MERIDIAN_OLD_VERSION env var"

assert_file_contains "$ROOT_DIR/setup.sh" "MERIDIAN_NEW_VERSION" \
    "setup.sh reads MERIDIAN_NEW_VERSION env var"

# bin/meridian.js has version command
assert_file_contains "$ROOT_DIR/bin/meridian.js" "version" \
    "meridian.js has version command"

assert_file_contains "$ROOT_DIR/bin/meridian.js" ".meridian-version" \
    "meridian.js reads .meridian-version file"
...
mkdir -p "$MOCK_HOME_V/.claude/meridian"
echo "1.1.1" > "$MOCK_HOME_V/.claude/meridian/.meridian-version"
