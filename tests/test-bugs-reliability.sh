...
# Test #7: check-global-state.sh delegates to meridian status
# The hook now calls `meridian status --write --quiet` instead of doing
# bash-level scanning. Scan root behavior is tested in test-rebuild-status.sh.
echo ""
echo "Issue #7: hook delegates to meridian status"
HOOK="$SCRIPT_DIR/specializations/claude-code/hooks/check-global-state.sh"
assert_contains "hook calls meridian status --write" "status --write" "$(cat "$HOOK")"
...
# Test hook runs without error (no meridian binary needed — falls back to local checkout)
echo ""
echo "Issue #7: hook runs without error"
bash "$HOOK" 2>/dev/null || true  # Should not crash even if meridian not found
pass; echo "  ✓ hook runs without error"

echo ""
echo "Issue #7: hook runs without error when meridian binary not found"
# Set HOME to empty dir so the local checkout path doesn't exist
EMPTY_DIR="$(mktemp -d)"
HOME_BAK="$HOME"
HOME="$EMPTY_DIR" bash "$HOOK" 2>/dev/null || true
pass; echo "  ✓ hook runs without error when meridian not installed"
rm -rf "$EMPTY_DIR"
