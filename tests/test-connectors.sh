...
TRANSPORT_TEST=$(MERIDIAN_SIMULATE=1 MERIDIAN_SIM_FIXTURES="$SCRIPT_DIR/simulation/fixtures/signals/github" node -e "
...
# Pre-configure
mkdir -p "$TEST_HOME/.claude/meridian"
cat > "$TEST_HOME/.claude/meridian/connectors.json" << 'CONNEOF'
{
  "github": {
    "transport": "simulate",
    "repos": [
      { "owner": "acme-corp", "repo": "webhook-service" }
    ],
...
export MERIDIAN_SIM_FIXTURES="$SCRIPT_DIR/simulation/fixtures/signals/github"

PULL_OUTPUT=$(node "$SCRIPT_DIR/bin/meridian.js" pull github --since 2026-02-20 2>&1) || true
...
SIGNAL_FILE="$TEST_HOME/.claude/meridian/signals/github/acme-corp/webhook-service/${TODAY_DATE}.md"
...
# Check summary file
SUMMARY_FILE="$TEST_HOME/.claude/meridian/signals/github/${TODAY_DATE}-summary.md"
...
SIGNALS_OUTPUT=$(node "$SCRIPT_DIR/bin/meridian.js" signals 2>&1) || true
...
ADD_OUTPUT=$(node "$SCRIPT_DIR/bin/meridian.js" pull github --add-repo acme-corp/NewService 2>&1) || true
assert_contains "add-repo confirms addition" "Added" "$ADD_OUTPUT"

# Verify it was added
CONFIG_CHECK=$(node -e "
const fs = require('fs');
const config = JSON.parse(fs.readFileSync('$TEST_HOME/.claude/meridian/connectors.json', 'utf8'));
...
# Test --remove-repo
REMOVE_OUTPUT=$(node "$SCRIPT_DIR/bin/meridian.js" pull github --remove-repo acme-corp/NewService 2>&1) || true
...
CONTAINER_RESULT=$(HOME="$TEST_CONTAINER_HOME" \
  ...
  MERIDIAN_GITHUB_REPOS="myorg/repo1,myorg/repo2" \
  ANTHROPIC_API_KEY="sk-ant-test" \
  node -e "
const fs = require('fs');
const path = require('path');
// Require meridian.js internals indirectly — call ensureContainerConfig via the module
// We simulate what the container does: set env vars and call the function
const meridianPath = '$SCRIPT_DIR/bin/meridian.js';
// Read the file and extract ensureContainerConfig + helpers
// Easier: just write a minimal connectors.json check script
const dir = path.join('$TEST_CONTAINER_HOME', '.claude', 'meridian');
fs.mkdirSync(dir, { recursive: true });
fs.writeFileSync(path.join(dir, 'connectors.json'), '{}');

// Now require meridian.js which defines ensureContainerConfig
...
mkdir -p "$TEST_PULL_HOME/.claude/meridian"
cat > "$TEST_PULL_HOME/.claude/meridian/connectors.json" << 'PULLEOF'
{
  "github": {
    "transport": "https",
    "token": "ghp_simulated",
    "repos": ["acme-corp/webhook-service"],
    "last_pull": null
  }
}
PULLEOF
...
PULL_ALL_OUTPUT=$(HOME="$TEST_PULL_HOME" \
  MERIDIAN_SIMULATE=1 \
  MERIDIAN_SIM_FIXTURES="$SCRIPT_DIR/simulation/fixtures/signals/github" \
  node "$SCRIPT_DIR/bin/meridian.js" pull --all --since 2026-02-20 2>&1) || true
