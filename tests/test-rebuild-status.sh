...
# gamma: team-state.md with "Current Sprint Focus" (Meridian style)
...
## What's Next
Run first meridian digest. Set up Monday cron.
EOF
...
# meridian status (default: print)
export AI_MEMORY_SCAN_ROOTS="$REPOS_ROOT"
CLI_OUTPUT=$(node "$REPO_ROOT/bin/meridian.js" status 2>&1 || true)
if echo "$CLI_OUTPUT" | grep -q "Cross-project status"; then
  _pass "meridian status prints table"
else
  _fail "meridian status prints table" "Output: $CLI_OUTPUT"
fi

# meridian status --json
JSON_OUTPUT=$(node "$REPO_ROOT/bin/meridian.js" status --json 2>&1 || true)
if echo "$JSON_OUTPUT" | node -e "JSON.parse(require('fs').readFileSync('/dev/stdin','utf8'))" 2>/dev/null; then
  _pass "meridian status --json returns valid JSON"
else
  _fail "meridian status --json returns valid JSON" "Invalid JSON output"
fi

# meridian status --write
node "$REPO_ROOT/bin/meridian.js" status --write 2>&1 || true
GLOBAL_AFTER=$(cat "$TEST_HOME/.claude/global-state.md")
if echo "$GLOBAL_AFTER" | grep -q "Alpha"; then
  _pass "meridian status --write rebuilds Active Projects"
else
  _fail "meridian status --write rebuilds Active Projects" "Alpha not found in global-state.md"
fi

# meridian status --write --quiet (no output)
QUIET_OUTPUT=$(node "$REPO_ROOT/bin/meridian.js" status --write --quiet 2>&1 || true)
if [ -z "$QUIET_OUTPUT" ]; then
  _pass "meridian status --write --quiet produces no output"
else
  _fail "meridian status --write --quiet produces no output" "Got: $QUIET_OUTPUT"
fi
