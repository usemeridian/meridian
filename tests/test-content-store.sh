...
export MERIDIAN_SIMULATE=1

# Create journal fixtures
JOURNAL_DIR="$TEST_HOME/.claude/memory/journal"
STORE_DIR="$TEST_HOME/.claude/meridian/content-store"
mkdir -p "$JOURNAL_DIR"

cat > "$JOURNAL_DIR/2026-02-24.md" << 'EOF'
## Meridian — Implement signal connectors
...
## Meridian — Digest engine and Slack delivery
...
## Meridian — Code review and bug fixes
...
")
if echo "$RESULT" | grep -qF "REPO:Meridian" && echo "$RESULT" | grep -qF "TITLE:Implement signal connectors" && echo "$RESULT" | grep -qF "WHY:Build the first signal channel"; then
...
RESULT=$(node -e "
  const cs = require('$REPO_ROOT/bin/content-store.js');
  const id1 = cs.generateEntryId('2026-02-24', 'Meridian', 'Test');
  const id2 = cs.generateEntryId('2026-02-24', 'Meridian', 'Test');
...
RESULT=$(node -e "
  const cs = require('$REPO_ROOT/bin/content-store.js');
  const tags = cs.extractTags({ repo: 'Meridian', title: 'Implement signal connectors', fields: {} });
  console.log(tags.join(','));
")
if echo "$RESULT" | grep -q "meridian" && echo "$RESULT" | grep -q "signal"; then
    _pass "tag extraction"
else
    _fail "tag extraction" "Expected meridian,signal in tags, got: $RESULT"
fi
...
RESULT=$(node -e "
  const cs = require('$REPO_ROOT/bin/content-store.js');
  cs.searchJournals('meridian', { storePath: '$STORE_DIR', limit: 2 }).then(results => {
    console.log('COUNT:' + results.length);
  });
")
...
RESULT=$(node -e "
  const cs = require('$REPO_ROOT/bin/content-store.js');
  const results = cs.searchText('Meridian digest', { storePath: '$TEXT_STORE' });
  console.log('COUNT:' + results.length);
  if (results.length > 0) {
...
RESULT=$(node -e "
  const cs = require('$REPO_ROOT/bin/content-store.js');
  const results = cs.queryMetadata({ storePath: '$STORE_DIR', repo: 'Meridian' });
  const allMatch = results.every(r => r.entry.repo === 'Meridian');
  console.log('ALL_MATCH:' + allMatch);
  console.log('COUNT:' + results.length);
")
...
RESULT=$(node -e "
  const cs = require('$REPO_ROOT/bin/content-store.js');
  const insights = cs.extractInsights({ storePath: '$STORE_DIR' });
  console.log('HAS_MERIDIAN:' + ('Meridian' in insights.repoActivity));
  console.log('REPOS:' + Object.keys(insights.repoActivity).length);
")
if echo "$RESULT" | grep -qF "HAS_MERIDIAN:true"; then
    _pass "repo activity"
else
    _fail "repo activity" "Missing Meridian in repo activity: $RESULT"
fi
...
RESULT=$(node "$REPO_ROOT/bin/meridian.js" index-journals --dir "$JOURNAL_DIR" --store "$STORE_DIR" 2>&1) || true
...
RESULT=$(node "$REPO_ROOT/bin/meridian.js" search-journals "signal" --text --store "$STORE_DIR" 2>&1) || true
...
RESULT=$(node "$REPO_ROOT/bin/meridian.js" insights --store "$STORE_DIR" 2>&1) || true
...
RESULT=$(node "$REPO_ROOT/bin/meridian.js" insights --json --store "$STORE_DIR" 2>&1) || true
...
## Meridian — Author attribution feature
...
**Author:** april
## Meridian — Product context review
...
")
if echo "$RESULT" | grep -qF "GREG:Meridian" && echo "$RESULT" | grep -q "NICK:" && echo "$RESULT" | grep -qF "APRIL:Meridian"; then
...
SIGNALS_DIR="$TEST_HOME/.claude/meridian/signals"
...
# ── Section 12: Repo exclusion (MERIDIAN_EXCLUDE_REPOS) ──────────────────────
echo ""
...
## meridian — worked on digest pipeline
...
# Each test runs in a fresh node process so MERIDIAN_EXCLUDE_REPOS is parsed fresh

# Index WITHOUT exclusion
RESULT=$(MERIDIAN_EXCLUDE_REPOS="" MERIDIAN_SIMULATE=1 node -e "
...
RESULT=$(MERIDIAN_EXCLUDE_REPOS="meridian,personal-project" MERIDIAN_SIMULATE=1 node -e "
  const cs = require('./bin/content-store');
  cs.indexJournals({ journalDir: '$EXCLUDE_JDIR', storePath: '$EXCLUDE_STORE2', embeddings: false })
    .then(r => console.log('COUNT:' + r.entryCount));
")
if echo "$RESULT" | grep -qF "COUNT:1"; then
    _pass "exclusion filters out meridian and personal-project"
else
    _fail "exclusion filters out meridian and personal-project" "Expected 1, got: $RESULT"
fi
...
RESULT=$(MERIDIAN_EXCLUDE_REPOS="meridian" MERIDIAN_SIMULATE=1 node -e "
  const cs = require('./bin/content-store');
  // Index all 3 first (EXCLUDE_REPOS is 'meridian' but we override at index by re-requiring)
  // Actually EXCLUDE_REPOS='meridian' is set, so meridian will be excluded at index too.
...
MERIDIAN_EXCLUDE_REPOS="" MERIDIAN_SIMULATE=1 node -e "
  const cs = require('./bin/content-store');
...
RESULT=$(MERIDIAN_EXCLUDE_REPOS="meridian" MERIDIAN_SIMULATE=1 node -e "
  ...
  console.log('HAS_MERIDIAN:' + repos.includes('meridian'));
  console.log('HAS_BROWSER:' + repos.includes('browser-app-v2'));
")
if echo "$RESULT" | grep -qF "HAS_MERIDIAN:false" && echo "$RESULT" | grep -qF "HAS_BROWSER:true"; then
    _pass "queryMetadata excludes meridian entries"
else
    _fail "queryMetadata excludes meridian entries" "Got: $RESULT"
fi
