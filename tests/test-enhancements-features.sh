#!/usr/bin/env bash
# Meridian — Test Suite for Feature Enhancements (#11, #12, #13)
# Tests:
...
mkdir -p "$MOCK_HOME/.claude/meridian"
...
# Copy doctor.sh into mock meridian
cp "$KIT_DIR/doctor.sh" "$MOCK_HOME/.claude/meridian/doctor.sh"
chmod +x "$MOCK_HOME/.claude/meridian/doctor.sh"

echo ""
echo "Meridian — Feature Enhancement Tests"
echo "═════════════════════════════════════════"
...
DOCTOR_OUTPUT="$(HOME="$MOCK_HOME" bash "$MOCK_HOME/.claude/meridian/doctor.sh" 2>&1)" && DOCTOR_EXIT=0 || DOCTOR_EXIT=$?
...
mkdir -p "$UNINSTALL_HOME/.claude/meridian"
touch "$UNINSTALL_HOME/.claude/meridian/doctor.sh"
...
# Verify meridian directory was removed
if [ ! -d "$UNINSTALL_HOME/.claude/meridian" ]; then
    pass "uninstall.sh removed meridian directory"
else
    fail "uninstall.sh did not remove meridian directory"
fi
