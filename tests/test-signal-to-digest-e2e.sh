...
export MERIDIAN_SIM_INTERCOM_FIXTURES="$REPO_ROOT/simulation/fixtures/signals/intercom"

MERIDIAN_DIR="$TEST_HOME/.claude/meridian"
SIGNALS_DIR="$MERIDIAN_DIR/signals"
STORE_DIR="$MERIDIAN_DIR/content-store"
JOURNAL_DIR="$TEST_HOME/.claude/memory/journal"

mkdir -p "$MERIDIAN_DIR" "$JOURNAL_DIR"
...
const dir = '$MERIDIAN_DIR';
const configFile = path.join(dir, 'connectors.json');
const config = {};

// GitHub — mirrors ensureContainerConfig
if (process.env.GITHUB_TOKEN && process.env.MERIDIAN_GITHUB_REPOS) {
  config.github = {
    transport: 'https',
    token: process.env.GITHUB_TOKEN,
    token_env: 'GITHUB_TOKEN',
    repos: process.env.MERIDIAN_GITHUB_REPOS.split(',').map(r => r.trim()),
    last_pull: null,
  };
}
...
# Pull GitHub signals
GITHUB_PULL=$(node "$REPO_ROOT/bin/meridian.js" pull github --since 2026-02-20 2>&1) || true
echo "$GITHUB_PULL" | grep -q "Issues:" && _pass "GitHub pull succeeded" || _fail "GitHub pull succeeded" "$GITHUB_PULL"

# Pull Intercom signals
INTERCOM_PULL=$(MERIDIAN_SIM_FIXTURES="$REPO_ROOT/simulation/fixtures/signals/intercom" \
  node "$REPO_ROOT/bin/meridian.js" pull intercom --since 2026-02-20 2>&1) || true
...
GITHUB_SIGNAL="$SIGNALS_DIR/github/acme-corp/webhook-service/${TODAY_DATE}.md"
...
PULL_ALL_OUTPUT=$(node "$REPO_ROOT/bin/meridian.js" pull --all --since 2026-02-20 2>&1) || true
