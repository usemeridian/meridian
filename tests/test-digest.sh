...
export MERIDIAN_SIM_FIXTURES="$REPO_ROOT/simulation/fixtures/signals/github"
...
mkdir -p "$TEST_HOME/.claude/meridian/signals/github"
cat > "$TEST_HOME/.claude/meridian/signals/github/2026-02-28-summary.md" << 'SIGEOF'
...
RESULT=$(node -e "
  const digest = require('$REPO_ROOT/bin/digest.js');
  const result = digest.collectSignals('2026-02-24', '$TEST_HOME/.claude/meridian/signals');
  console.log(result);
")
...
cat > "$TEST_HOME/.claude/meridian/signals/github/2026-01-15-summary.md" << 'SIGEOF'
# Old Summary
...
RESULT=$(node -e "
  const digest = require('$REPO_ROOT/bin/digest.js');
  const result = digest.collectSignals('2026-02-01', '$TEST_HOME/.claude/meridian/signals');
  console.log(result);
")
...
mkdir -p "$TEST_HOME/.claude/meridian/signals/github/TestOrg/TestRepo"
cat > "$TEST_HOME/.claude/meridian/signals/github/TestOrg/TestRepo/2026-02-28.md" << 'SIGEOF'
...
SLACK_FILE="$TEST_HOME/.claude/meridian/digests/2026-02-28-slack-engineering.json"
...
mkdir -p "$TEST_HOME/.claude/meridian/digests/engineering"
mkdir -p "$TEST_HOME/.claude/meridian/digests/product"
cat > "$TEST_HOME/.claude/meridian/digests/engineering/2026-02-28.md" << 'DEOF'
# Engineering Digest

Test engineering digest content.
DEOF
cat > "$TEST_HOME/.claude/meridian/digests/product/2026-02-28.md" << 'DEOF'
# Product Digest
...
ENG_SLACK="$TEST_HOME/.claude/meridian/digests/2026-02-28-slack-engineering.json"
PROD_SLACK="$TEST_HOME/.claude/meridian/digests/2026-02-28-slack-product.json"
