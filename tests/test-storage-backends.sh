...
RESULT=$(MERIDIAN_STORAGE_BACKEND=json node -e "
...
RESULT=$(MERIDIAN_STORAGE_BACKEND=sqlite node -e "
  const cs = require('$REPO_ROOT/bin/content-store.js');
...
MERIDIAN_STORAGE_BACKEND=json MERIDIAN_SIMULATE=1 node -e "
  const cs = require('$REPO_ROOT/bin/content-store.js');
...
JSON_COUNT=$(MERIDIAN_STORAGE_BACKEND=json node -e "
  const { getBackend, clearCache } = require('$REPO_ROOT/bin/storage');
  clearCache();
...
JSON_INSIGHTS=$(MERIDIAN_STORAGE_BACKEND=json node -e "
...
")
SQLITE_INSIGHTS=$(MERIDIAN_STORAGE_BACKEND=sqlite node -e "
  const { clearCache } = require('$REPO_ROOT/bin/storage');
  clearCache();
  const cs = require('$REPO_ROOT/bin/content-store.js');
  const i = cs.extractInsights({ storePath: '$MIGRATION_STORE' });
