---
description: Set up Meridian team context for your organization. Interactive walkthrough that creates a team, sets up profiles, creates a team context repo, configures Slack digests, sets up Notion integration, and initializes product-state.md for a pilot repo. Run once per team.
---
...
meridian whoami --setup

# Create a new team (generates a shareable team ID)
meridian team create

# Or join an existing team
meridian team join <team-id>

# Check your profile and team status
meridian whoami
meridian team status
```

These commands create local config files in `~/.claude/meridian/`. The full
...
1. Meridian is installed (`~/.claude/meridian/` exists or `meridian version` works)
2. User has a Meridian profile (`meridian whoami` -- if not, run `meridian whoami --setup`, which also asks for Slack user ID)
3. User has a team configured (`meridian team status` -- if not, run `meridian team create`)
...
<repo>/
  ...
  meridian.json          # Shared config (webhook URLs, model, excluded repos)
...
Shared decision trail for [team name]. Powered by [Meridian](https://github.com/usemeridian/meridian).
...
Team members use Meridian in their daily AI-assisted development sessions.
...
Once the team context repo exists, link it so Meridian knows where to find shared
context files:

```bash
# Link the team context repo (sets context_repo in config)
meridian context init <path-to-team-context-repo>

# Distribute shared context files to engineer repos
meridian context sync
```

`meridian context sync` copies files from `context/` in the team context repo into
...
If certain repos should be excluded from digests and context sync (e.g., the meridian
repo itself), set:

```bash
export MERIDIAN_EXCLUDE_REPOS="meridian,other-repo"
```

This can also be configured in `meridian.json` in the team context repo.
...
Suggest defaults based on the team's configured personas (see `meridian personas`).
  ...
  digest:
    ...
    steps:
      ...
      - name: Install Meridian
        run: npm install -g meridian-dev

      - name: Reindex journals
        run: meridian reindex

      - name: Generate and deliver digest
        env:
          SLACK_WEBHOOK_URL: ${{ secrets.SLACK_WEBHOOK_URL }}
        run: meridian digest --since last-week --deliver
...
2. **Create child pages**:
   ...
   Adapt these pages to match the team's configured personas (see `meridian personas`).
...
Check the team's configured personas (read from `~/.claude/meridian/personas.json` or fall back to `templates/personas.json`). For personas that have state templates (e.g. product-state.md, strategy-state.md), offer to create them.
...
Each team member's journals need to flow to the team context repo. Meridian handles
this with a single command:

```bash
meridian journal sync
```
...
Tell the user to ensure their session-end Stop hook includes `meridian journal sync`.
The hook is configured in `~/.claude/settings.json` and typically runs
`meridian reindex --conversations-only --export` followed by `meridian journal sync`.
...
has no user profile claiming it, Meridian will use autopilot prompt templates
...
with `meridian autopilot status`."**

Show them the current autopilot status:

```
meridian autopilot status
```
...
```
meridian autopilot disable <persona-id>
```

They can re-enable it later with `meridian autopilot enable <persona-id>`.
