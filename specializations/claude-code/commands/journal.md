...
Look for `~/.claude/meridian/journal-summary.sh`. If found, run it based on what the user asked:

```bash
# This week (default)
bash ~/.claude/meridian/journal-summary.sh

# Last week
bash ~/.claude/meridian/journal-summary.sh --last-week

# Specific date range
bash ~/.claude/meridian/journal-summary.sh --from 2026-02-01 --to 2026-02-28

# All history as Markdown (good for Notion/GitHub)
bash ~/.claude/meridian/journal-summary.sh --all --format markdown

# Team aggregate (each subdir is a contributor's journal dir)
bash ~/.claude/meridian/journal-summary.sh --team ~/team-journals

# Custom journal directory
bash ~/.claude/meridian/journal-summary.sh --dir ~/.ai-memory/memory/journal
```
...
Or install manually:
```bash
cp ~/repos/greg/meridian/journal-summary.sh ~/.claude/meridian/journal-summary.sh
chmod +x ~/.claude/meridian/journal-summary.sh
```
