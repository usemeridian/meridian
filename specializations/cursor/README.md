# Cursor Integration — Meridian

Full setup guide for Cursor users.

## Quick Setup

Install Meridian globally, then run the Cursor initializer:
```bash
...
| `/init-memory` command | ✓ | ✗ Use `meridian init-cursor` or `setup.sh --tool cursor --repo .` |
