...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --version)
            VERSION_FILE="$HOME/.claude/meridian/.meridian-version"
            if [ -f "$VERSION_FILE" ]; then
                echo "Meridian v$(cat "$VERSION_FILE")"
            else
                echo "Meridian version unknown (no .meridian-version file found)"
            fi
            exit 0
            ;;
        --help) usage; exit 0 ;;
...
echo ""
echo "Installing Meridian for: $TOOL"
...
MERIDIAN_OLD_VERSION="${MERIDIAN_OLD_VERSION:-}"
MERIDIAN_NEW_VERSION="${MERIDIAN_NEW_VERSION:-}"
if [ "$UPDATE" = true ] && [ -n "$MERIDIAN_OLD_VERSION" ] && [ -n "$MERIDIAN_NEW_VERSION" ]; then
    if [ "$MERIDIAN_OLD_VERSION" != "$MERIDIAN_NEW_VERSION" ]; then
        info "Upgrading from v${MERIDIAN_OLD_VERSION} to v${MERIDIAN_NEW_VERSION}"
    fi
fi
...
case "$TOOL" in
    claude-code) PERSONAS_DIR="$HOME/.claude/meridian" ;;
    *)           PERSONAS_DIR="$HOME/.ai-memory/meridian" ;;
esac
...
case "$TOOL" in
    claude-code)
        ...
        # doctor.sh script
        run mkdir -p "$HOME/.claude/meridian"
        DOCTOR_DEST="$HOME/.claude/meridian/doctor.sh"
        ...
        JOURNAL_DEST="$HOME/.claude/meridian/journal-summary.sh"
...
if [ -n "$MERIDIAN_NEW_VERSION" ]; then
    INSTALL_VERSION="$MERIDIAN_NEW_VERSION"
...
if [ -n "$INSTALL_VERSION" ]; then
    KIT_DEST_DIR="$HOME/.claude/meridian"
    VERSION_DEST="$KIT_DEST_DIR/.meridian-version"
    if [ "$DRY_RUN" = false ]; then
        ...
        info "[dry-run] Would write v${INSTALL_VERSION} to $KIT_DEST_DIR/.meridian-version"
    fi
fi
...
echo ""
echo "Meridian installed for $TOOL."
echo ""
echo "Next steps:"
...
if [ "$TOOL" = "claude-code" ]; then
    ...
    echo "  5. Set up your profile: meridian whoami --setup"
    echo "  6. Create or join a team: meridian team create"
